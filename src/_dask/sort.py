...
def sort(data_size, file_type):
    ...
    with LocalCluster(processes=process, memory_limit='14GB') as cluster:
        with Client(cluster) as client:
            ...
            ans = df.sort_values(by=sort_colunm)
            if file_type == "csv":
                ans.to_csv(f"./dask_output/{file_type}/sort/{data_size}", index=False)
            elif file_type == "parquet":
                ans.to_parquet(f"./dask_output/{file_type}/sort/{data_size}", engine="fastparquet", overwrite=True)
            else:
                raise ValueError(f"File type {file_type} not supported")

            t = timeit.default_timer() - t_start
            m = memory_usage()
            write_log('sort', 'dask', data_size, file_type, t, m)
