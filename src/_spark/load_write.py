...
def load_write(data_size, file_type):
    ...
    if file_type == "csv":
        (
            ...
            .option("header", True)
            .mode('overwrite')
            .csv(f"./spark_output/{file_type}/load_write/{data_size}")
        )
    elif file_type == "parquet":
        (
            ans.write
            .mode('overwrite')
            .parquet(f"./spark_output/{file_type}/load_write/{data_size}")
        )
    else:
        raise ValueError(f"File type {file_type} not supported")
    ...
    m = memory_usage()
    write_log('load_write', 'spark', data_size, file_type, t, m)
    spark.stop()
    del df
    del ans
